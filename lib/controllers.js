 ...
 * @param {*} options 
 */
var generateController = (schema, options) => {
  //console.log(`${JSON.stringify(schema.schema)}`);
  var controller = {};
  if (_.isUndefined(schema) || _.isEmpty(schema)) {
    return controller;
  }
  ...
  controller.getById = (request, reply) => {
    schema.model.findOne({_id: request.params.id}, {'__v': 0}, (err, doc) => {
      if (err) {
        return reply().code(500).send();
      }
      reply(doc);
  ...
  controller.update = (request, reply) => {
    schema.model.update({_id: request.params.id},
      {'$set': request.payload}, 
      {upsert: true},
      (err, doc) => {
        if (err) {
  ...
  controller.remove = (request, reply) => {
    schema.model.deleteOne({_id: request.params.id}, (err, doc) => {
      if (err) {
        return reply().code(500).send();
      }
      reply(doc);
